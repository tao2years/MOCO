...
import MOCO.Edge;
import MOCO.Node;
import VirtualDevice.SmartSpeaker;
import graph.Graph;
...
public class SmartSpeaker_Model {
    ...
    private static List<API> getAllPossibleAPIs() {
        ...
        apis.add(new API("playTrack", true, false));
        apis.add(new API("stopTrack", true, false));
        apis.add(new API("setVolume0", true, false));
        apis.add(new API("setVolume50", true, false));
        return apis;
    }

    private static String executeApi(String api, SmartSpeaker smartSpeaker) {
        switch (api) {
            case "turnOn":
                return smartSpeaker.turnOn().equals("success") ? smartSpeaker.toString() : "skip";
            case "turnOff":
                return smartSpeaker.turnOff().equals("success") ? smartSpeaker.toString() : "skip";
            case "playTrack":
                return smartSpeaker.playTrack("Track1").equals("success") ? smartSpeaker.toString() : "skip";
            case "stopTrack":
                return smartSpeaker.stopTrack().equals("success") ? smartSpeaker.toString() : "skip";
            case "setVolume0":
                return smartSpeaker.setVolume(0).equals("success") ? smartSpeaker.toString() : "skip";
            case "setVolume50":
                return smartSpeaker.setVolume(50).equals("success") ? smartSpeaker.toString() : "skip";
            default:
                return smartSpeaker.toString();
        }
    }
    ...
    private static void exploreState(String state, String systemState, Set<Edge> edges) {
        ...
        for (API api : apis) {
            SmartSpeaker currentDevice = SmartSpeaker.fromString(state);
            String nextState = executeApi(api.getName(), currentDevice);
            if (!nextState.equals("skip")) {
                SmartSpeaker nextDevice = SmartSpeaker.fromString(nextState);
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            SmartSpeaker_Model test = new SmartSpeaker_Model();
        ...
        String dotFile = "src/main/java/MOCO/modelFiles/smartspeaker_output.dot";
        generateDotFile(dotSet, nodeMap.size(), dotFile);

        Collection<graph.Edge> edge = userGraph.getEdges();
        String fileName = "src/main/java/MOCO/modelFiles/smartspeaker.json";
