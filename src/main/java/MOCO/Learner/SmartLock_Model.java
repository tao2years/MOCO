...
import MOCO.Edge;
import MOCO.Node;
import VirtualDevice.SmartLock;
import graph.Graph;
...
public class SmartLock_Model {
    private static final Logger LOGGER = LogManager.getLogger();
    private static Map<String, Set<String>> stateTransitions;

    public SmartLock_Model() {
        stateTransitions = new HashMap<>();
    }

    private static List<API> getAllPossibleAPIs() {
        List<API> apis = new ArrayList<>();
        apis.add(new API("lock", true, false));
        apis.add(new API("unlock", true, false));
        return apis;
    }

    private static String executeApi(String api, SmartLock smartLock) {
        switch (api) {
            case "lock":
                return smartLock.lock().equals("success") ? smartLock.toString() : "skip";
            case "unlock":
                // Assuming the correct password is "1234" for the purpose of this model
                return smartLock.unlock("1234").equals("success") ? smartLock.toString() : "skip";
            default:
                return smartLock.toString();
        }
    }

    private static void generateGraph(Set<Edge> edges) {
        SmartLock device = new SmartLock();
    ...
    private static void exploreState(String state, String systemState, Set<Edge> edges) {
        ...
        for (API api : apis) {
            SmartLock currentDevice = SmartLock.fromString(state);
            String nextState = executeApi(api.getName(), currentDevice);
            if (!nextState.equals("skip")) {
                SmartLock nextDevice = SmartLock.fromString(nextState);
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            SmartLock_Model test = new SmartLock_Model();
        ...
        String dotFile = "src/main/java/MOCO/modelFiles/smartlock_output.dot";
        generateDotFile(dotSet, nodeMap.size(), dotFile);

        Collection<graph.Edge> edge = userGraph.getEdges();
        String fileName = "src/main/java/MOCO/modelFiles/smartlock.json";
