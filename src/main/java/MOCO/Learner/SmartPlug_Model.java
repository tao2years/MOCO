...
import MOCO.Edge;
import MOCO.Node;
import VirtualDevice.SmartPlug;
import graph.Graph;
...
public class SmartPlug_Model {
    ...
    private static List<API> getAllPossibleAPIs() {
        ...
        apis.add(new API("setTimer0", true, false));
        apis.add(new API("setTimer50", true, false));
        return apis;
    }

    private static String executeApi(String api, SmartPlug smartPlug) {
        switch (api) {
            case "turnOn":
                return smartPlug.turnOn().equals("success") ? smartPlug.toString() : "skip";
            case "turnOff":
                return smartPlug.turnOff().equals("success") ? smartPlug.toString() : "skip";
            case "setTimer0":
                return smartPlug.setTimer(0).equals("success") ? smartPlug.toString() : "skip";
            case "setTimer50":
                return smartPlug.setTimer(50).equals("success") ? smartPlug.toString() : "skip";
            default:
                return smartPlug.toString();
        }
    }

    private static void generateGraph(Set<Edge> edges) {
        SmartPlug device = new SmartPlug();
    ...
    private static void exploreState(String state, String systemState, Set<Edge> edges) {
        ...
        for (API api : apis) {
            SmartPlug currentDevice = SmartPlug.fromString(state);
            String nextState = executeApi(api.getName(), currentDevice);
            if (!nextState.equals("skip")) {
                SmartPlug nextDevice = SmartPlug.fromString(nextState);
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            SmartPlug_Model test = new SmartPlug_Model();
        ...
        String dotFile = "src/main/java/MOCO/modelFiles/smartplug_output.dot";
        generateDotFile(dotSet, nodeMap.size(), dotFile);

        Collection<graph.Edge> edge = userGraph.getEdges();
        String fileName = "src/main/java/MOCO/modelFiles/smartplug.json";
